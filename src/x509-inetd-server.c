    ...
    if (pid < 0) {
        ...
        while (requestsServed++ < 1000) {
            ...
            if (SSL_accept(ssl) == FAIL) /* do SSL-protocol accept */
                ...
                if (strlen(buffer) > 1) {
                    tt.offset_in += sprintf(&tt.buffer_in[tt.offset_in],
                            "Input:%s\n", buffer);
                }
...
//    }
    ...
    unsigned char ssl_session_ctx_id = 0;

    SSL_CTX_set_session_id_context(ctx, &ssl_session_ctx_id,
            sizeof(ssl_session_ctx_id));

    SSL_CTX_set_timeout(ctx, 999);

    // Default flags is to create a ssl reuse caching server,
    int flags = SSL_SESS_CACHE_SERVER;

    // Finally, set the session-mode.
    if (test != SSL_CTX_set_session_cache_mode(ctx, flags)) {
        printf("error: Could not set session mode");
        exit(EXIT_FAILURE);
    }
    ...
    if (1
            != SSL_CTX_set_cipher_list(ctx,
                    "AES128-SHA:RC4:AES:CAMELLIA128-SHA:!ADH:!aNULL:!DH:!EDH:!eNULL:!LOW:!SSLv2:!EXP:!NULL")) {
        printf("error: Could not set cihpers");
        exit(EXIT_FAILURE);
    }
